...
## final plot of detection function
```{r}
plot_detfc <- distdata %>% 
  ggplot(aes(x = distance)) +
	geom_histogram(aes(y = ..count../10, fill = factor(seaState), text = paste0(..count..," sharks\n", "Sea state: ", factor(seaState))), binwidth = 0.02, col="white", alpha = .3) +
	geom_line(data = data.frame(x = dist, y = proba_detec0), aes(x = x, y = y, text = paste0("Sea state: ", factor(seaState), "\n Detection probability: ", proba_detec0)), col = "#54C3C7", size = 1.5) +
	geom_line(data = data.frame(x = dist, y = proba_detec1), aes(x = x, y = y, text = paste0("Sea state: ", factor(seaState), "\n Detection probability: ", proba_detec1)), col = "#3C8C8F", size = 1.5) +
	geom_line(data = data.frame(x = dist, y = proba_detec2), aes(x = x, y = y, text = paste0("Sea state: ", factor(seaState), "\n Detection probability: ", proba_detec2)), col = "#2B6466", size = 1.5) +
	geom_line(data = data.frame(x = dist, y = proba_detec3), aes(x = x, y = y, text = paste0("Sea state: ", factor(seaState), "\n Detection probability: ", proba_detec3)), col = "#1A3C3D", size = 1.5) +
	# Nom des axes
	xlab("Distance (km)") +
...
for (session_choix in 2:3){
	for (availability_choix in c("1", "041", "shelf")){
		# créé png
		eval(parse(text = paste0('png("./shiny-app/www/images/dsm_s', session_choix, 
														 '_av', availability_choix, '.pred.png", width = 700, height = 1000)')))
		# plot
