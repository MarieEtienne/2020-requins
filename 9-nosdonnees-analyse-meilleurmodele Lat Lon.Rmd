...
```{r import_donnees}
load("C:/Users/rindr/Desktop/2020_requins/PRIGLA_envrt/data/effort_output.RData")
# effort_ouput : Effort data divided in multiple sub-data differing by their scale of study.

load("C:/Users/rindr/Desktop/2020_requins/PRIGLA_envrt/data/list_prepare_obs_by_sp.RData")
# list_prepare_obs_by_sp : observation data divided in multiple sub-data differing by their scale of study.

load("C:/Users/rindr/Desktop/2020_requins/PRIGLA_envrt/data/predata_output_2.RData")
# predata_output : A grid of 2km cells size with different covariable associated around the observation.

gridata <- read.dbf("C:/Users/rindr/Desktop/2020_requins/PRIGLA_envrt/data/SPEE_CAPECET_Grid2km_modified.dbf")
...
xy <- data.frame(predata[,1:2])
coordinates(xy) <- c("latitude", "longitude")
proj4string(xy) <- CRS("+init=epsg:2154")  ## for example

res <- spTransform(xy, CRS("+init=epsg:2154"))
res
...
x = seq(0,0.3, 0.001)
y = 1-exp(-(x/detfc.sea.hr$ddf$par[1])^(-detfc.sea.hr$ddf$par[2]))
plot(x,y)
```
...
```{r}
predata_tmp2 <- predata_output$predata %>% filter(session == 2)
predata_tmp3 <- predata_output$predata %>% filter(session == 3)
...
[1] "-------- Session 2 --------"
[1] "Avec availaibility = 1, on estime qu'il y a 4488 requins au total dans la zone."
[1] "Avec availaibility = 0.41, on estime qu'il y a 10947 requins au total dans la zone."
[1] "Avec availaibility selon on/off shelf, on estime qu'il y a 7088 requins au total dans la zone.
...
## Carte pour la session 2 et availability = 1

```{r}
(dsm_s2_av1.pred %>% sort(decreasing = TRUE))[0:50]
```
...
zone_obs_map + 
	geom_raster(data = as.data.frame(cbind(x = predata_tmp2$longitude, y = predata_tmp2$latitude, abondance = dsm_s2_av1.pred)),
...
```{r}
library(corrplot)
corrplot(cor(segdata[,16:23]), method="circle")
corrplot(cor(predata[,6:14]), method="circle")
```
