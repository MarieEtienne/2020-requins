...
## Centrage-réduction des covariables
```{r}
# distdata
...
<!-- # Fonction de densité qui marche pas -->
...
<!-- * Graphe distCoast : pas du tout précis, les lignes de l'intervalle de confiance montrent des valeurs variant fortement pour la pente -->
<!-- * Graphe POC_4w_mean : plus précis mais presque plat, très peu d'effet, et à nouveau intervalle de confiance montre qu'on pourrait dire que ça un impact positif ou négatif  -->

<!-- Conclusion : pas fou le modèle en sélectionnant différemment les covariables avec les données de chaque session. Nous allons plutôt sélectionner les covariables du modèle sur toutes les données puis ajuster les paramètres par session. -->
...
```{r} 
zone_obs_map + 
	geom_raster(data = as.data.frame(cbind(x = predata$longitude, y = predata$latitude, abondance = dsm_s2_av1.pred)),
...
zone_obs_map + 
	geom_raster(data = data_pred_carto %>% filter(abondance < 20),
							aes(x = x, y = y, fill = abondance)) + 
  coord_fixed(ratio = 1) +
	geom_path(
		data = segdata,
